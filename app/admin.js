...
// pregunto si existe dicho inicio de sesion y si es diferente al correo admin
if((storedAuthUser == null) || (storedAuthUser && storedAuthUser.email !== 'admin@admin.com')){
...
// obtengo el id mas grande que tengo en mi Array de P y S y le sumo uno, apply() es para aplicar la función a un conjunto de valores  del arreglo
let idMax = Math.max.apply(Math, storedPyS.map(id => {return id.id;}))
...
function editar(id) {
  ...
  document.getElementById('publicado').checked = storedPyS[posisionPyS].publicado ;
